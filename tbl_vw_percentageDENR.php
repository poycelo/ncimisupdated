  ...
  <div class="mt-3">
                ...
                <table id="DENR" class="table-bordered display text-xs" style="width:100%;">
                        ...
                        <?php
                            $query_inter = $dbConn->query("SELECT * FROM tbl_interventions WHERE SID = '$SID' AND INFO_MAIN LIKE '%RESOURCES%'");
                        ...
                        <tr>
                             ...
                             <td>
                                <?php 
                                $Delquantity = number_format($row_inter["INFO_DEL_QUANTITY"]);
                                $Infoquantity = number_format($row_inter["INFO_COMM_QUANTITY"]);
                        ...
                        <tr>
                           <td hidden></td>
                            ...
                            <td><b>
                            <?php
                              $query_comm = $dbConn->query("SELECT sum(INFO_COMM_QUANTITY) as TotalCommQ FROM tbl_interventions WHERE SID = '$SID' AND INFO_MAIN LIKE '%RESOURCES%'");
                            ...
                            <td><b>
                            <?php
                               $query_comm = $dbConn->query("SELECT sum(INFO_DEL_QUANTITY) as TotalDelQ FROM tbl_interventions WHERE SID = '$SID' AND INFO_MAIN LIKE '%RESOURCES%'");
                              $row_comm = mysqli_fetch_assoc($query_comm);
                            ...
                            <td><b>
                            <?php
                              $query_comm = $dbConn->query("SELECT sum(INFO_COMM_BUDGET) as TotalBudget FROM tbl_interventions WHERE SID = '$SID' AND INFO_MAIN LIKE '%RESOURCES%'");
                              $row_comm = mysqli_fetch_assoc($query_comm);
                            ...
                            <td><b>
                            <?php
                              $query_comm = $dbConn->query("SELECT sum(INFO_ALLOC_BUDGET) as TotalAlloc FROM tbl_interventions WHERE SID = '$SID' AND INFO_MAIN LIKE '%RESOURCES%'");
                              $row_comm = mysqli_fetch_assoc($query_comm);
                            ...
                            <td><b>
                            <?php
                              $query_comm = $dbConn->query("SELECT sum(INFO_DISBURSED_BUDGET) as TotalDisbursed FROM tbl_interventions WHERE SID = '$SID' AND INFO_MAIN LIKE '%RESOURCES%'");
                              $row_comm = mysqli_fetch_assoc($query_comm);
