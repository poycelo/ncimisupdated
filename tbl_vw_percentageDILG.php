  <!-- Interventions: Department of Interior and Local Government (DILG) -->
  <div class="mt-3">
                <div class="form-row mt-3">
                    <div class="col-lg-12">
                        <h5 class="custom-font">Department of Interior and Local Government</h5>
                    </div>
                </div>
                ...
                <table id="DILG" class="table-bordered display text-xs" style="width:100%;">
                        ...
                        <?php
                            $query_inter = $dbConn->query("SELECT * FROM tbl_interventions WHERE SID = '$SID' AND INFO_MAIN LIKE '%INTERIOR%'");
                        ...
                        <tr>
                           <td hidden></td>
                            ...
                            <td><b>
                            <?php
                              $query_comm = $dbConn->query("SELECT sum(INFO_COMM_QUANTITY) as TotalCommQ FROM tbl_interventions WHERE SID = '$SID' AND INFO_MAIN LIKE '%INTERIOR%'");
                            ...
                            <td><b>
                            <?php
                               $query_comm = $dbConn->query("SELECT sum(INFO_DEL_QUANTITY) as TotalDelQ FROM tbl_interventions WHERE SID = '$SID' AND INFO_MAIN LIKE '%INTERIOR%'");
                              $row_comm = mysqli_fetch_assoc($query_comm);
                            ...
                            <td><b>
                            <?php
                              $query_comm = $dbConn->query("SELECT sum(INFO_COMM_BUDGET) as TotalBudget FROM tbl_interventions WHERE SID = '$SID' AND INFO_MAIN LIKE '%INTERIOR%'");
                              $row_comm = mysqli_fetch_assoc($query_comm);
                            ...
                            <td><b>
                            <?php
                              $query_comm = $dbConn->query("SELECT sum(INFO_ALLOC_BUDGET) as TotalAlloc FROM tbl_interventions WHERE SID = '$SID' AND INFO_MAIN LIKE '%INTERIOR%'");
                              $row_comm = mysqli_fetch_assoc($query_comm);
                            ...
                            <td><b>
                            <?php
                              $query_comm = $dbConn->query("SELECT sum(INFO_DISBURSED_BUDGET) as TotalDisbursed FROM tbl_interventions WHERE SID = '$SID' AND INFO_MAIN LIKE '%INTERIOR%'");
                              $row_comm = mysqli_fetch_assoc($query_comm);
